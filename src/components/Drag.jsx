...
// as로 별칭 주기 -> lucide 이름변경은 X, 별칭(대문자 시작) 사용가능. 별칭만들면 꼭 사용해야함
...
const Drag = () => {
  return (
    <Section secTitle='home Drag home'>
      ...
      <div className="drag-wrap">
        <div className="drag-deco">
          {dragDeco.slice(0, 6).map((item) => (
            <img src={item.img} alt={`deco${item.id}`} key={item.id}/>
          ))}
        </div>
      ...
      <button type="button" className="ico saved">
        <SavedIco  />
      </button>
    </Section>
  )
